...
I learnt about the relation of a state variable defined via `useState` and the React component it is defined "within", or "asssociated to", and how that means that when a state variable changes its value, this re-renders the React component, which in turn means that the function that defines the React component is re-run with the new state value.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=01%3A%20useState%3A%20greeting&em=jaime.ferbec%40gmail.com).

...
It was good to be reminded that, even though I'm writing something very similar to HTML, it's JSX, and that compiles esentially to `React.createElement`s, which could be then, in turn, rendered by any React UI engine. But if I know I'm going to render to a web page in a web browser, this exercises definitely helps to get a first glance at an optimal, clean approach. And in the way it reminded me of one of the most frequent uses of `useRef`, as well as the React lifecycle of execution flow, with the use of that `useEffect` to initialize the Vanilla library in the appropiate moment.

This exercise made a question come up in my head: if I were to render to a differente React UI engine, I should be able to leverage `useRef` and `useEffect` to get similar effects (apply some native operations on the native translation of a React component), right?
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=05%3A%20useRef%20and%20useEffect%3A%20DOM%20interaction&em=jaime.ferbec%40gmail.com).

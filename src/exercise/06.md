...
I learnt how to put all the knowledge acquired so far to build a more realistic app that uses asynchronous HTTP calls to a server, and properly manages errors.

It was also good to build a React Component when building a custom `ErrorBoundary`, to get a sense of how React used to be built.

I got a good sense on when to group state together (when it has some sort of relation, end hence it is set at the same time), which is a good prelude to `useReducer`, and when and how to use it.

It was also good to be reminded of another use for the `key` prop: to mount and unmount a Component when necessary.
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=06%3A%20useEffect%3A%20HTTP%20requests&em=jaime.ferbec%40gmail.com).
